    ...
    <div class="main">
        ...
        <mu-paper :zDepth="1" class="pay">
            ...
            <form @submit.prevent="handlePaySubmit" action="api.php" method="get" target="_blank">
                <mu-text-field label="各位行行好吧" hintText="请输入支付金额" type="text" labelFloat class="my-input mb0" v-model="amount"/>

                <mu-row gutter class="payway">
                    <mu-col width="100" tablet="100" desktop="100" v-for="way in payway">
                        <a href="javascript:void(0)" class="shortcut-tile">
                            <div class="icon-default" :class="'icon-'+ way.name + (chooseWay == way.value?' choose':'')" @click="chooseWay=way.value"></div>
                        </a>
                    </mu-col>
                </mu-row>
